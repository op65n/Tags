...
public final class HexUtil {
     ...
     */
    private static String getCaptureGroup(final Matcher matcher, final String group) {
        try {
     ...
     */
    public static String colorify(final String message) {
    ...
    private static String parseRainbow(final String message) {
        ...
        while (matcher.find()) {
            final StringBuilder parsedRainbow = new StringBuilder();

            // Possible parameters and their defaults
            int speed = -1;
            float saturation = 1.0F;
            float brightness = 1.0F;
            final boolean looping = getCaptureGroup(matcher, "looping") != null;

            final String speedGroup = getCaptureGroup(matcher, "speed");
            if (speedGroup != null) {
                try {
            ...
            final String saturationGroup = getCaptureGroup(matcher, "saturation");
            ...
            final String brightnessGroup = getCaptureGroup(matcher, "brightness");
            ...
            final int stop = findStop(parsed, matcher.end());
            final String content = parsed.substring(matcher.end(), stop);
            ...
            final int length = looping ? Math.min(contentLength, CHARS_UNTIL_LOOP) : contentLength;
            final ColorGenerator rainbow;
            if (speed == -1) {
            ...
            for (int i = 0; i < chars.length; i++) {
                ...
                if (c == '&' && i + 1 < chars.length) {
                    ...
                    final org.bukkit.ChatColor color = org.bukkit.ChatColor.getByChar(next);
            ...
            final String after = parsed.substring(stop);
    ...
    private static String parseGradients(final String message) {
        String parsed = message;

        Matcher matcher = GRADIENT_PATTERN.matcher(parsed);
        while (matcher.find()) {
            final StringBuilder parsedGradient = new StringBuilder();

            int speed = -1;
            final boolean looping = getCaptureGroup(matcher, "loop") != null;

            final List<Color> hexSteps = Arrays.stream(getCaptureGroup(matcher, "hex").substring(1).split(":"))
    ...
    private static String parseHex(final String message) {
        String parsed = message;

        for (final Pattern pattern : HEX_PATTERNS) {
            Matcher matcher = pattern.matcher(parsed);
            while (matcher.find()) {
                final String color = translateHex(cleanHex(matcher.group()));
                final String before = parsed.substring(0, matcher.start());
    ...
    private static String parseLegacy(final String message) {
     ...
     */
    private static int findStop(final String content, final int searchAfter) {
        final Matcher matcher = STOP.matcher(content);
        while (matcher.find()) {
    ...
    private static String cleanHex(final String hex) {
     ...
     */
    private static String translateHex(final String hex) {
        return ChatColor.of(hex).toString();
    }

    private static String translateHex(final Color color) {
    ...
    private enum ChatColorHexMapping {
        ...
        ChatColorHexMapping(final int hex, final ChatColor chatColor) {
            this.r = (hex >> 16) & 0xFF;
            this.g = (hex >> 8) & 0xFF;
            this.b = hex & 0xFF;
    ...
    public static class Gradient implements ColorGenerator {
        ...
        public Gradient(final List<Color> colors, final int steps) {
            if (colors.size() < 2)
            ...
            final float increment = (float) this.steps / (colors.size() - 1);
        ...
        public Color next() {
            ...
            if (this.gradients.size() < 2) {
                ...
                final float segment = (float) this.steps / this.gradients.size();
                final int index = (int) Math.min(Math.floor(adjustedStep / segment), this.gradients.size() - 1);
        ...
        private static class TwoStopGradient {
            ...
            private TwoStopGradient(final Color startColor, final Color endColor, final float lowerRange, final float upperRange) {
             ...
             */
            public Color colorAt(final int step) {
                return new Color(
            ...
            private int calculateHexPiece(final int step, final int channelStart, final int channelEnd) {
    ...
    public static class AnimatedGradient extends Gradient {
        ...
        public AnimatedGradient(final List<Color> colors, final int steps, final int speed) {
            super(colors, steps);
    ...
    public static class Rainbow implements ColorGenerator {
        ...
        public Rainbow(final int totalColors, final float saturation, final float brightness) {
            if (totalColors < 1)
    ...
    public static class AnimatedRainbow extends Rainbow {
        ...
        public AnimatedRainbow(final int totalColors, final float saturation, final float brightness, final int speed) {
